  ...
  return (
    <details className="group border-b border-white/8 py-5">
      <summary className="flex items-center justify-between cursor-pointer list-none gap-4">
        <span className="font-body font-semibold text-chalk text-base">{q}</span>
        <span className="shrink-0 w-6 h-6 border border-white/15 flex items-center justify-center text-mist group-open:border-brand/40 group-open:text-brand transition-colors">
...
export default function ProduktyPage() {
  return (
    <div className="pt-20">
      {/* Hero */}
      <section className="relative bg-dirt overflow-hidden py-24 md:py-36">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] pointer-events-none"
          style={{ background: "radial-gradient(circle, #D4780A 0%, transparent 70%)", opacity: 0.08 }} />
        <div className="max-w-[1200px] mx-auto px-6 md:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className="font-syne font-black text-chalk leading-[0.88] mb-5"
        ...
        <div className="max-w-[1200px] mx-auto px-6 md:px-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-white/5 border border-white/5">
            {PRODUCT_SPECS.map((spec, i) => (
              <div key={i} className="bg-[#130F0C] p-6 md:p-8 hover:bg-dirt transition-colors duration-200">
                <p className="font-tech text-[10px] tracking-[0.15em] uppercase text-mist/40 mb-2">{spec.label}</p>
                <p className="font-tech text-chalk text-2xl md:text-3xl font-medium leading-none">{spec.value}</p>
              </div>
            ))}
          </div>
          <div className="mt-10 p-6 border border-white/5 flex flex-wrap gap-4 items-center">
            ...
            {["IEC 60529 (IP69K)", "EN 62493", "CE", "RoHS 3"].map((c) => (
              <span key={c} className="font-tech text-xs text-mist border border-white/10 px-3 py-1.5 tracking-wide">{c}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="bg-dirt py-20 md:py-28">
        <div className="max-w-[1200px] mx-auto px-6 md:px-12">
          <div className="flex flex-col gap-16">
            {features.map((f, i) => (
              <div key={i} className={`grid grid-cols-1 lg:grid-cols-2 gap-10 items-center ${i % 2 === 1 ? "lg:grid-flow-dense" : ""}`}>
                <div className={i % 2 === 1 ? "lg:col-start-2" : ""}>
                  <h3 className="font-syne font-black text-chalk text-xl md:text-2xl leading-tight mb-4">{f.title}</h3>
                  <p className="text-mist leading-relaxed">{f.desc}</p>
                </div>
                <div className={`relative aspect-[4/3] overflow-hidden border border-white/5 ${i % 2 === 1 ? "lg:col-start-1" : ""}`}>
      ...
      <section className="bg-[#130F0C] py-20 md:py-28">
        <div className="max-w-[1200px] mx-auto px-6 md:px-12">
          <h2 className="font-syne font-black text-chalk leading-[0.88] mb-10"
            style={{ fontSize: "clamp(34px, 4.5vw, 64px)" }}>
            Dostupné provedení.
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {variants.map((v) => (
              <div key={v.code} className="bg-dirt border border-white/5 p-5 hover:border-brand/20 transition-colors group">
                ...
                <p className="font-tech text-chalk text-sm">{v.len} · {v.k}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              ...
              <div className="divide-y divide-white/5">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between py-3 gap-4">
                    ...
                    <span className="font-tech text-sm text-chalk text-right">{val}</span>
                  </div>
                ))}
              </div>
            </div>
            <div>
              <div className="flex flex-col gap-6 mb-10">
                {[
                  ...
                  <div key={num} className="flex gap-5">
                    ...
                    <div>
                      <h4 className="font-body font-semibold text-chalk text-sm mb-1">{title}</h4>
              ...
              <div className="border border-brand/20 p-6">
                <p className="font-body font-semibold text-chalk mb-2">Chcete vzorové svítidlo?</p>
